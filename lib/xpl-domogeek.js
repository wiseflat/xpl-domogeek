...
var proto = {
	...
	_log: function() {
        ...
        writeConfig: function(body) {
                ...
                jf.writeFile(self.configFile, JSON.stringify(self.config), function(err) {
                        if(err) {
                            self.sendNoConfig();
                        } else {
                            self.readConfig();
        ...
        readConfig: function(callback) {
                var self = this;
                jf.readFile(self.configFile, { encoding: "utf-8"}, function (err, body) {
        ...
        sendCommands: function(url, callback){
                ...
                if (typeof self.config !== 'undefined'){
                        self.config.forEach(function(item, index) {
                                if(item.enable) self.sendHttpRequest(item.type, item.url);
                        });   
                }
        },    
        ...
        validConfigSchema: function(body, callback) {
                var self = this;
                console.log("info type="+body.type);
                if (typeof(body.type) !== "string") {
                        console.log("error type="+body.type);
                        return false;
                }
                console.log("info url="+body.url);
                if (typeof(body.url) !== "string") {
                        console.log("error url="+body.url);
                        return false;
                }
                console.log("info enable="+body.enable);
                if (typeof(body.enable) !== "string") {
                        console.log("error enable="+body.enable);
                        return false;
                }
                return true;
        }
