...
var proto = {
	...
	_log: function(log) {
		...
		self.configHash.city = evt.body.city;
		self.configHash.area = evt.body.area;
		self.configHash.schoolarea = evt.body.schoolarea;
		self.configHash.postcode = evt.body.postcode;
			...
			else self._sendXplStat(self.configHash, 'domogeek.config', evt.header.source);
                });
        },
        ...
        sendBasic: function(callback) {
				...
				item.url = item.url.replace('_city_',self.configHash.city);
				item.url = item.url.replace('_area_',self.configHash.area);
				item.url = item.url.replace('_schoolarea_',self.configHash.schoolarea);
				item.url = item.url.replace('_postcode_',self.configHash.postcode);
